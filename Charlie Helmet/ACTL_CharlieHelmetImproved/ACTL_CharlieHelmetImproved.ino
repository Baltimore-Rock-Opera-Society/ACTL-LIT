...
const byte wifiSelect = 10;
//const byte sdSelect = 53;
const byte attempts = 3; // number of tries at connection
const unsigned long checkstatus = 30000; // check status every 30 seconds (5 minutes by

unsigned long oldTime;

IPAddress ip; // ip address of wifi101
int quality; // connection quality
int wifistatus = WL_IDLE_STATUS;    // the Wifi radio's status

boolean report = true; // Serial output?

void die(){ // no functioning wifi 101 card
  if  (report) Serial.print(F("Error: No WIFI card"));
// any warning process  
  while(1);
}
...
void setup() {
  ...
  if (wifistatus == WL_NO_SHIELD) die();
  ...
  if (!ssidConnect(WIFI_SSID, attempts) ) {
    if (report) Serial.println(F("Abort"));

    while(1){ // connection has failed
    // error message, bang a drum, etc
    delay(3000);
    }
  }
...
void loop() {
  ...
  if (timeNow - oldTime >= checkstatus){ // check wifi every 5 minutes (checkstatus = 300000)
  oldTime = timeNow; // reset timeout
  wifistatus = WiFi.status();
    if(wifistatus == WL_NO_SHIELD) die();

    if( (wifistatus == WL_CONNECTED) || (wifistatus > WL_DISCONNECTED) ) {
    quality = WiFi.RSSI();
    }else{ // reconnect if not AP_listening, AP_connected , AP_failed or dead!
      while (WiFi.status() != WL_CONNECTED){
        if(report) Serial.println(F("WiFi down"));

        if (ssidConnect(WIFI_SSID, attempts)){ // try to re-connect
        oldTime = millis();
        break;
        }else{
          if(report) Serial.println(F("Re-trying in 20 seconds"));
        delay(10000); // // 10 seconds (20) seconds to trying again
        }
      }
    }

    if(report) {
    Serial.print(F("WiFi on, quality "));
    Serial.print(quality); Serial.println(F(" dBm"));
    }  
  }
...
boolean ssidConnect(String thessid, byte attempts){
  byte mytry = 0;
  // attempt to connect to Wifi network:
  do { // try attempts times
  mytry ++;
    if (report) {
  ...
  // Connect to network.
  wifistatus = WiFi.begin(thessid, WIFI_PASS);
  ...
  } while (wifistatus != WL_CONNECTED && mytry < attempts); // 3 attempts
