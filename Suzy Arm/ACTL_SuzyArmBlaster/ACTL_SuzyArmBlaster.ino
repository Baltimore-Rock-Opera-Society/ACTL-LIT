...
#define SUZY_BLASTER_LED_COUNT 16

// How bright should the NeoPixels be? (0-255)
#define SUZY_BLASTER_LED_BRIGHTNESS 255 

// Declare our NeoPixel strip object:
Adafruit_NeoPixel strip(SUZY_BLASTER_LED_COUNT, LED_PIN, NEO_GRB + NEO_KHZ800);
...
uint32_t timeBlue = strip.Color(0, 104, 240);
uint32_t blue = strip.Color(50, 100, 200);
...
unsigned int suzyBlasterState = 0;
...
void setup() {
  ...
  strip.setBrightness(SUZY_BLASTER_LED_BRIGHTNESS); // Set BRIGHTNESS to about 1/5 (max = 255)
...
// This function will be called by handleOSCMessage()
void suzyBlaster(OSCMessage &msg) {
  suzyBlasterState = msg.getInt(0);

  Serial.print("/suzyblaster: ");
  Serial.println(suzyBlasterState);
}
...
void handleOSCMessage() {
  ...
  if (size > 0) {
    ...
    if (!msg.hasError()) {
      ...
      msg.dispatch("/suzyblaster", suzyBlaster);
...
void loop() {
  ...
  switch (suzyBlasterState) {
  case 0:
    // statements
    // Turn off the arm blaster NeoPixel ring!
    strip.clear();
    strip.show();
    break;
  case 1:
    // statements
    // Turn on the arm blaster! This is the "fire" mode,
    // probably needs an animation here (TO DO)
    strip.fill(timeBlue, 0, SUZY_BLASTER_LED_COUNT);
    strip.show();
    delay(500);
    suzyBlasterState = 0;
    break;
